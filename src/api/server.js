...
app.get('/forecast/:city?', async (req, res) => {
  ...
  if (city) {
    const forecastDataRes = await fetch(
      `http://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${API_KEY}&units=metric&lang=es&cnt=5`
    );
    ...
    const location = await response.json();
    const forecastDataRes = await fetch(
      `http://api.openweathermap.org/data/2.5/forecast?q=${location.city}&appid=${API_KEY}&units=metric&lang=es&cnt=5`
    );
    const forecastData = await forecastDataRes.json();
    res.send({ forecastData });
  }
